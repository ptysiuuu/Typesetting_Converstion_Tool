...
def test_cli_help():
    result = subprocess.run(
        ["python3", "cli/main.py", "--help"], capture_output=True, text=True
    )
...
def test_cli_missing_file():
    result = subprocess.run(
        ["python3", "cli/main.py", "non_existent.md", "--format", "latex"],
        capture_output=True,
        text=True,
    )
...
def test_cli_input_without_output(tmp_path):
    ...
    result = subprocess.run(
        ["python3", "cli/main.py", str(input_file), "--format", "latex"],
        capture_output=True,
        text=True,
    )
...
def test_cli_output_without_input():
    result = subprocess.run(
        ["python3", "cli/main.py", "--format", "latex", "--output", "out.tex"],
        capture_output=True,
        text=True,
    )
